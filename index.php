...
while($row = $result->fetch_assoc()) {
  echo "<button class=\"dropdown-item\" type=\"button\" onClick=\"artData(" .($row["artworkid"]-1) .")\">" .$row["title"] . "</button>";
}
...
while($row = $result->fetch_assoc()) {
  echo "<button class=\"dropdown-item\" type=\"button\" onClick=\"museumData(" .($row["museumid"]-1) .")\">" .$row["name"] . "</button>";
}
...
while($row = $result->fetch_assoc()) {
  echo "<button class=\"dropdown-item\" type=\"button\" onClick=\"artistData(" .($row["artistid"]-1) .")\">" .$row["fullname"] . "</button>";
}
